# Direct-mode unit tests for ToxiGuard.  Run: pytest tests/direct/ -v
import json

CONTRACT = "contracts/toxiguard.py"
...
def test_submit_and_evaluate(direct_vm, direct_deploy, direct_alice):
    ...
    assert int(idx) == 0
    direct_vm.mock_llm(r".*JSON.*", json.dumps({"label": "safe", "score": 80, "reasoning": "ok"}))
    c.evaluate(0)
    item = c.get_item(0)
    assert item["label"] == "safe"
