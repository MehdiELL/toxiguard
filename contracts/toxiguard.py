...
# ToxiGuard - AI content moderation on GenLayer.
...
class ToxiGuard(gl.Contract):
    ...
    def evaluate(self, item_id: int) -> None:
        ...
        def leader_fn():
            ...
            prompt = f"""You are a strict but fair content moderator. Judge the CONTENT for toxicity, harassment, and hate speech.
...
Classify the CONTENT as exactly one of: safe, toxic, hateful.
Also give an integer severity score from 0 to 100, and a one-sentence reason.

Respond with ONLY JSON: {{"label": "<one of: safe, toxic, hateful>", "score": <integer 0-100>, "reasoning": "<one sentence>"}}"""
